...
class FixWangLandau : public Fix {
 public:
  FixWangLandau(class LAMMPS *, int, char **);
  ~FixWangLandau() override;
 ...
 private:
  ...
  double patomtrans, pmoltrans, pmolrotate, pmctot, f;
  std::vector<double> qs, hs;    // Wang Landau qs and hs
  std::vector<int> ns;           // Number of particles in each Wang Landau bin 
  std::map<int, int> n2i;        // Map from number to bin index 
